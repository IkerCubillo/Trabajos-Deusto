...
public class GestorAgenda {
	 ...
	 * @return	Lista de slots de agendaf
	 */
	...
	public void login() {
		...
		// Tarea 2
		borrarAgendaActual(false);
		if(mapaAgendas.containsKey(nick)) {
		...
		System.out.println("Usuarios acialmente en agenta:");
		for(String usuario: mapaAgendas.keySet()) {
			ArrayList<EspacioAgenda> lista = mapaAgendas.get(usuario);
			System.out.println(usuario + " - " + lista.size() + lista );
		}
	}
	...
	private void cargaDatosFichero( File f ) {
		...
		try {
			...
			if(objetoLeido instanceof ArrayList) {
				listaAgenda = (ArrayList<EspacioAgenda>) objetoLeido;
			}else {
				mapaAgendas = (HashMap<String, ArrayList<EspacioAgenda>>) objetoLeido;
			}
			reiniciaVentanaDeAgenda();
		} catch (Exception e) {
		...
		reiniciaVentanaDeAgenda();  // Llamar a este reinicio después de la carga
	}
