...
import examen202205.datos.ConPrecio;
...
public class VentanaVisualizacionRegistros extends VentanaGrafica {
	...
	double precioMinimo;  // valor del precio mínimo de todos los registros con precio
	double precioMaximo;  // valor del precio máximo de todos los registros con precio
	int yPrecioMin; // y del precio menor
	int yPrecioMax; // y del precio mayor
	public void sacaEstadisticaDePrecios() {
		calculaYDibujaLimitesPrecios();  // Este método ya hace el cálculo de los ejes y los atributos de precios
		if (precioMaximo==0) {  // No hay precios en el sistema - no se puede dibujar
			return;
		}
		// T4 
		// TODO  Queda pendiente dibujar los precios partiendo del mapa, y calcular y dibujar los precios medios por día
		//       (Puedes usar los métodos de cálculo de x,y que ya están programados aquí abajo)
	}
	
	// Devuelve la coordenada X para un precio de una fecha dada, ya calculadas las variables de pantalla
	// Si la fecha es incorrecta devuelve la x inicial
	private double getXPrecioDia( String fechaAAAAMMDD ) {
		Date fechaDia;
		try {
			fechaDia = Utils.FORMATEADOR_FECHA_AAAAMMDD_HM.parse( fechaAAAAMMDD + " 12:00" );
			int numDiasDesdeInicio = (int) Math.round( (fechaDia.getTime() - fechaInicial.getTime()) / (24*3600000.0) );
			return xInicial + numDiasDesdeInicio * pixelesDia;
	...
	// Devuelve la coordenada Y para un precio de un día-hora dado, ya calculadas las variables de pantalla
	private double getYPrecioDia( double precio ) {
		return Utils.proyectar( precio, precioMinimo, precioMaximo, yPrecioMin, yPrecioMax, true );
	}
	
	private void calculaYDibujaLimitesPrecios() {
		borra();
		// Calcula precios extremos
		precioMinimo = Double.MAX_VALUE;  // Muy grande (cualquiera será menor que este)
		precioMaximo = 0;  // Muy pequeño (cualquiera será mayor que este)
		for (Registro r : grupoRegistros.getListaRegistros()) {
			if (r instanceof ConPrecio) {
				ConPrecio regPrecio = (ConPrecio) r;
				if (regPrecio.getPrecio() < precioMinimo) {
					precioMinimo = regPrecio.getPrecio();
				}
				if (regPrecio.getPrecio() > precioMaximo) {
					precioMaximo = regPrecio.getPrecio();
				}
			}
		}
		if (precioMaximo==0) {  // No hay precios en el sistema - no se puede dibujar
		...
		// Calcula el resto de variables de dibujado de precios, manteniendo las que hacen el dibujado de registros
		yPrecioMin = y24;  // Los límites son los mismos que las horas, pero en sentido inverso (precio mayor hacia arriba, menor abajo)
		yPrecioMax = y0;
		// Dibuja los ejes horizontales
		dibujaLinea( 0, yInicial, getAnchura(), yInicial, 2.0f, Color.BLACK );  // Barra superior
		dibujaLinea( xInicial, yPrecioMin, xInicial+pixelesDia*(numDias-1), yPrecioMin, 1.0f, Color.LIGHT_GRAY );  // Marca de precio mínimo
		dibujaLinea( xInicial, yPrecioMax, xInicial+pixelesDia*(numDias-1), yPrecioMax, 1.0f, Color.LIGHT_GRAY );  // Marca de precio máximo
		dibujaLinea( 0, yFinal, getAnchura(), yFinal, 2.0f, Color.BLACK );  // Barra inferior
		// Rotula las fechas y las horas
		int xDia = xInicial;
		Date fechaDia = new Date( fechaInicial.getTime() );
		...
		dibujaTextoCentrado( 0, yPrecioMin-30, xInicial, 60, String.format( "%.3f", precioMinimo ), FONT_TEXTO_ROTULOS, Color.BLACK, true );
		dibujaTextoCentrado( 0, yPrecioMax-30, xInicial, 60, String.format( "%.3f", precioMaximo ), FONT_TEXTO_ROTULOS, Color.BLACK, true );
	}
